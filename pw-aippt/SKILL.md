...
1. **背景颜色改变**
   - 问题: AI 自动改变了背景颜色(如白色变成紫蓝色渐变)
   - 原因: 提示词中"整体配色方案"描述太模糊,AI 自由发挥了
   - 解决:
     - **约束背景**: 在"不可改区域"中明确写从模板提取的背景信息
     - **允许自由**: 在"可改区域"中明确写"背景可自由设计"

   ```markdown
   # 约束背景(推荐)
   【不可改区域】(严格保持原样)
   - 背景: 纯白色背景,不要添加任何渐变或装饰
   - 配色: 红色标题,黑色正文

   # 或者允许自由发挥
   【可改区域】
   - 背景: 可自由设计背景颜色和装饰,但需保持专业风格
   ```

   **关键**: 分析模板时就要提取背景信息(颜色、渐变、装饰),然后在提示词中明确约束或明确放开,不要模糊不清。

2. **装饰元素消失或改变**
   - 问题: 模板的装饰图标、边框等元素消失或样式改变
   - 原因: 没有明确列出所有装饰元素
   - 解决: 详细列出每个装饰元素的位置和样式
...
3. **提示词精确度原则**
   - ❌ 模糊: "整体配色方案和布局结构"
   - ✅ 精确: "白色背景,红色标题,黑色正文,左右分栏布局"
   - ❌ 模糊: "医疗相关的装饰元素"
   - ✅ 精确: "左侧白色医疗图标(十字、心电图、DNA),背景模糊的听诊器图片"

4. **字体大小一致性**
   - 问题: 不同页面的标题、正文字体大小不一致
   - 原因: 没有明确指定字体大小约束
   - 解决: 在所有提示词中统一字体大小规范
   ...
   - 标题字体: 48pt 粗体
   - 正文字体: 24pt 常规
   - 作者信息: 18pt,格式为 "@用户名" (不要加"作者:"前缀)
   ```

   **关键**: 所有页面使用相同的字体大小规范,确保视觉一致性。
